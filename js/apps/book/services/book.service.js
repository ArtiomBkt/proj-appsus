...
const API_BOOKS = 'API_BOOKS'
const API_KEY = 'AIzaSyDDO5jb_bxNabjaf6nAaLgnn3DzQpqbiqI'
...
function getBooksOpts(search) {
  // let apiBooks = utilService.loadFromStorage(API_BOOKS)
  // if (!apiBooks) {
  console.log('calling the API')
  return axios
    .get(
      `https://www.googleapis.com/books/v1/volumes?printType=books&q=${search}&key=${API_KEY}`
    )
    .then((res) => {
      // utilService.saveToStorage(API_BOOKS, res)
      // apiBooks = res.data.items
      return Promise.resolve(res.data.items)
    })
  // }
  return Promise.resolve(apiBooks.data.items)
}
...
function addGoogleBook(googleBook) {
  const book = {
    ...
    listPrice: {
      amount: Math.floor(Math.random() * 178) + 1,
      currencyCode: 'ILS',
      isOnSale: parseInt(Math.random() * 2) ? true : false,
    },
    reviews: [],
  }
